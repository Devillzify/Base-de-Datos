...
select NAME,SURNAMES from members where CITY = 'Manacor' and PHONE is null;
...
select NAME from publishers where STATE is not null and CITY is null;
...
select concat(NAME,SURNAMES) as fullname,MEMBER_CODE from members order by MEMBER_CODE desc limit 1;

-- 16. Return the day we registered the oldest member (does not have to be the number 1 member)
SELECT ENTRY_DATE,NAME,MEMBER_CODE FROM MEMBERS WHERE ENTRY_DATE IS NOT NULL ORDER BY ENTRY_DATE ASC LIMIT 1;
...
select avg(timestampdiff(day,BORROW_DATE,RETURN_DATE)) from borrows where RETURN_DATE >= BORROW_DATE and year(RETURN_DATE) !=0;


-- 20. Names and surnames of the members who live in Manacor or Son Servera and who were registered in 2011.
select NAME,SURNAMES,CITY from members where (city = 'Manacor' or city = 'Son Servera') and year(ENTRY_DATE) = 2011;

-- 21. Title, full name of the author and name of the publisher of all books (2 solutions)
select TITLE,concat(a.NAME,' ',a.SURNAMES)as fullname,p.NAME publisher from books b
...
select TITLE as libro,p.NAME as publisher_name,concat(a.NAME,' ',a.SURNAMES)as author from books b 
left join publishers p on p.PUBLISHER_CODE = b.PUBLISHER_CODE
left join authors a on a.AUTHOR_CODE = b.AUTHOR_CODE where b.LANGUAGE = 'CA';
...
select NAME,SURNAMES,city from members where MEMBER_CODE in(select MEMBER_CODE from borrows where BORROW_DATE is not null and RETURN_DATE is null);
...
join borrows bo on b.BOOK_CODE = bo.COPY_CODE where bo.MEMBER_CODE = 1;
...
select distinct p.NAME,g.GENRE_CODE,b.TITLE from books b
join genrebook g on b.BOOK_CODE = g.BOOK_CODE
join publishers p on p.PUBLISHER_CODE = b.PUBLISHER_CODE where g.GENRE_CODE = 'HIST';
...
select conctat(a.NAME,' ',a.SURNAMES)as authors from authors a
join publishers p on a.AUTHOR_CODE = p.PUBLISHER_CODE;
